...
class TokenStore:
    """Manages gateway auth codes and access tokens."""
    ...
    def exchange_code(
        ...
        """Exchange an authorization code for a gateway access token.
        ...
        logger.debug("Issued gateway token for session %s", entry.session_id[:8])
        return token_entry
    ...
    def _prune(self) -> None:
        ...
        if expired_codes or expired_tokens:
            logger.debug(
                "Pruned %d codes, %d tokens", len(expired_codes), len(expired_tokens)
            )
